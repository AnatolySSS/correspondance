...
const SET_DATA = "correspondance/corr-fu-reducer/SET_DATA";
const SET_UPLOAD_STATUS = "correspondance/corr-fu-reducer/SET_UPLOAD_STATUS";
...
const corrFuReducer = (state = initialState, action) => {
  switch (action.type) {
    case SET_DATA:
      return {
        ...state,
...
export const requestData = () => {
  return (dispatch) => {
    DataAPI.getData("corrFu").then((data) => {
...
export const updateData = (rowData, rowId) => {
  return (dispatch) => {
    DataAPI.updateData("corrFu", rowData, rowId).then((message) => {
      DataAPI.getData("corrFu").then((data) => {
...
export const deleteData = (rowId) => {
  return (dispatch) => {
    DataAPI.deleteData("corrFu", rowId).then((message) => {
      DataAPI.getData("corrFu").then((data) => {
        dispatch(setData(changeDateFormat(data), message.message));
      });
    });
...
export const addData = (rowData) => {
  return (dispatch) => {
    DataAPI.addData("corrFu", rowData).then((data) => {
      DataAPI.getData("corrFu").then((data) => {
        dispatch(setData(changeDateFormat(data)));
      });
    });
...
export const uploadData = (data) => {
  return (dispatch) => {
    DataAPI.uploadData("corrFu", data).then((data) => {
      dispatch(setUploadStatus(true));
    });
  };
};

export default corrFuReducer;
